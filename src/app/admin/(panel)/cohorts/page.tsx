...
export default async function AdminCohortsPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        <h1 style={{ color: "var(--atext2)", fontSize: 22, fontWeight: 800, margin: "0 0 4px", letterSpacing: "-0.03em" }}>Cohort Analysis</h1>
      ...
      {rows.length === 0 ? (
        <div style={{ background: "var(--am1)", border: "1px solid var(--aw09)", borderRadius: 18, minHeight: 170, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10 }}>
          <div style={{ width: 50, height: 50, borderRadius: "50%", background: "var(--aw04)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <GitFork size={22} color="var(--aw2)" strokeWidth={1.4} />
          </div>
        ...
        <div style={{ background: "var(--asurface1)", border: "1px solid var(--aw07)", borderRadius: 16, overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 560 }}>
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={r.cohortStart} style={{ borderBottom: i < rows.length - 1 ? "1px solid var(--aw04)" : "none" }}>
                  <td style={{ padding: "13px 18px", color: "var(--atext2)", fontSize: 13, fontWeight: 600, whiteSpace: "nowrap" }}>{r.cohortLabel}</td>
                  <td style={{ padding: "13px 18px", color: "var(--aw5)", fontSize: 13 }}>{r.size}</td>
                  {[r.w1, r.w2, r.w3, r.w4].map((pct, wi) => (
                    <td key={wi} style={{ padding: "13px 18px" }}>
                      {pct === null ? (
                        <span style={{ color: "var(--aw15)", fontSize: 12.5 }}>-</span>
                      ) : (
                        <span style={{
