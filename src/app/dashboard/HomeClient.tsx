  ...
  return (
    <div style={card}>
      <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        <p style={{ color: "rgb(250,250,250)", fontSize: 15, fontWeight: 700, margin: 0 }}>Your schedule today</p>
      </div>
      {schedule.length === 0 ? (
      ...
      <div className="home-stat-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
        <Link href="/dashboard/appointments" style={{ textDecoration: "none" }}>
          <div style={{ ...card, padding: "20px 20px 18px", cursor: "pointer" }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", margin: "0 0 6px" }}>TODAY&apos;S BOOKINGS</p>
            <p style={{ color: "rgb(250,250,250)", fontSize: 28, fontWeight: 800, margin: "0 0 4px", letterSpacing: "-0.04em" }}>{todayBookingsCount}</p>
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, margin: 0 }}>{nextAppointmentTime ? `Next at ${nextAppointmentTime}` : "Nothing left today"}</p>
          </div>
        </Link>
        ...
        <Link href="/dashboard/daily-brief" style={{ textDecoration: "none" }}>
          <div style={{ ...card, padding: "20px 20px 18px", cursor: "pointer", border: needsYou.length > 0 ? "1px solid rgba(248,113,113,0.22)" : card.border }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", margin: "0 0 6px" }}>NEEDS YOUR EYES</p>
            <p style={{ color: needsYou.length > 0 ? "rgb(248,113,113)" : "rgb(250,250,250)", fontSize: 28, fontWeight: 800, margin: "0 0 4px", letterSpacing: "-0.04em" }}>{needsYou.length}</p>
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{topNeed ? topNeed.text : "All clear"}</p>
          </div>
        </Link>
        ...
        <Link href="/dashboard/settings" style={{ textDecoration: "none" }}>
          <div style={{ ...card, padding: "20px 20px 18px", cursor: "pointer" }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", margin: "0 0 6px" }}>FAMILY HOURS</p>
            {familyHoursEnabled ? (
              <>
                <p style={{ color: "rgb(250,250,250)", fontSize: 28, fontWeight: 800, margin: "0 0 4px", letterSpacing: "-0.04em" }}>
                  {familyHoursStreak} <span style={{ fontSize: 16, color: "rgba(255,255,255,0.4)", fontWeight: 600 }}>day streak</span>
                </p>
                <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, margin: 0 }}>Protected daily</p>
              </>
            ) : (
              <>
                <p style={{ color: "rgb(250,250,250)", fontSize: 28, fontWeight: 800, margin: "0 0 4px", letterSpacing: "-0.04em" }}>Off</p>
                <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, margin: 0 }}>Tap to set up</p>
              </>
            )}
          </div>
        </Link>
        ...
        <Link href="/dashboard/operations" style={{ textDecoration: "none" }}>
          <div style={{ ...card, padding: "20px 20px 18px", cursor: "pointer" }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", margin: "0 0 6px" }}>THIS MONTH</p>
            <p style={{ color: "rgb(250,250,250)", fontSize: 28, fontWeight: 800, margin: "0 0 4px", letterSpacing: "-0.04em" }}>{fmtPrice(monthRevenue)}</p>
            <p style={{ color: monthRevenueDeltaPct == null ? "rgba(255,255,255,0.35)" : monthRevenueDeltaPct >= 0 ? "rgb(52,211,153)" : "rgb(248,113,113)", fontSize: 12, margin: 0 }}>
          ...
          {paymentNeed && (
            <div style={{ margin: "12px 16px 0", padding: "11px 14px", background: "rgba(245,158,11,0.07)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 9 }}>
                ...
                <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 12.5, margin: 0, lineHeight: 1.5 }}>{paymentNeed.text}</p>
              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ ...card, padding: "18px 20px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                { label: "View Team",      href: "/dashboard/team",         color: "rgba(255,255,255,0.05)",  text: "rgba(255,255,255,0.7)" },
              ].map(a => (
                <Link key={a.label} href={a.href} style={{ display: "block", padding: "10px 14px", borderRadius: 10, background: a.color, border: "1px solid rgba(255,255,255,0.07)", color: a.text, fontSize: 13, fontWeight: 700, textDecoration: "none", textAlign: "center" }}>
                  {a.label}
                </Link>
              ))}
            </div>
          ...
          <div style={{ background: "rgba(139,92,246,0.06)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 14, padding: "16px 18px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
              ...
              <span style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase" }}>Needs your attention</span>
            </div>
