...
const STATUS_LABEL: Record<AppointmentStatus, { label: string; color: string; bg: string }> = {
  completed: { label: "Done",      color: "rgba(255,255,255,0.35)", bg: "rgba(255,255,255,0.05)" },
  ...
  cancelled: { label: "Cancelled", color: "rgba(255,255,255,0.25)", bg: "rgba(255,255,255,0.03)" },
};
...
function ClientProfile({ client, appointments, engaged, onClose }: {
  ...
  const statItem = (label: string, value: string, valColor?: string) => (
    <div style={{ textAlign: "center" }}>
      <p style={{ color: valColor ?? "rgb(250,250,250)", fontSize: 18, fontWeight: 800, margin: "0 0 2px", letterSpacing: "-0.02em" }}>{value}</p>
  ...
  const actionBtn = (Icon: React.ElementType, label: string, href: string | null, primary?: boolean) => {
    ...
    return (
      <a href={href ?? undefined} style={{
        ...
        border: `1px solid ${primary ? "rgba(139,92,246,0.4)" : "rgba(255,255,255,0.08)"}`,
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px 24px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 14, padding: "14px 16px", gap: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <p style={{ color: "rgb(250,250,250)", fontSize: 13.5, fontWeight: 700, margin: 0 }}>{fmtDate(upcoming.starts_at)} · {upcoming.service_name}</p>
                    </div>
                  </div>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(248,113,113,0.06)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 14, padding: "14px 16px", gap: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <p style={{ color: "rgba(255,255,255,0.55)", fontSize: 13, margin: 0 }}>Last seen {fmtDaysAgo(client.daysSinceLastVisit).toLowerCase()}</p>
                    </div>
                  </div>
              ...
              <div style={{ background: "rgba(139,92,246,0.06)", border: "1px solid rgba(139,92,246,0.18)", borderRadius: 14, padding: "14px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                  ...
                  <span style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>Insight</span>
                </div>
                ...
                <div style={{ ...card, padding: "14px 16px" }}>
                  ...
                  {[{ Icon: Mail, val: client.email }, { Icon: Phone, val: client.phone }].filter(c => c.val).map(({ Icon, val }) => (
                    <div key={val} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
                      <Icon size={12} color="rgba(255,255,255,0.3)" style={{ flexShrink: 0 }} />
                      <span style={{ color: "rgba(255,255,255,0.55)", fontSize: 12.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{val}</span>
                    </div>
                  ))}
                  {!client.email && !client.phone && <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 12.5, margin: 0 }}>No contact info on file</p>}
                </div>
                <div style={{ ...card, padding: "14px 16px" }}>
          ...
          {tab === "History" && (
            <div>
              <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", margin: "0 0 10px" }}>
                {past.length} completed visit{past.length === 1 ? "" : "s"}
              </p>
              {past.length === 0 ? (
                <div style={{ ...card, padding: "40px 20px", textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: 13 }}>No completed visits yet</div>
              ) : (
                <div style={{ ...card, overflow: "hidden" }}>
                  {past.map((v) => {
                    const s = STATUS_LABEL[v.status];
                    return (
                      <div key={v.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 16px", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ color: "rgb(250,250,250)", fontSize: 13, fontWeight: 700, margin: "0 0 2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{v.service_name}</p>
                          <p style={{ color: "rgba(255,255,255,0.32)", fontSize: 11.5, margin: 0 }}>{fmtDate(v.starts_at)}{v.stylist_name ? ` · ${v.stylist_name}` : ""}</p>
                        </div>
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          <p style={{ color: "rgb(52,211,153)", fontSize: 13, fontWeight: 700, margin: "0 0 2px" }}>{fmtPrice(Number(v.price))}</p>
                          {v.tip_amount ? <p style={{ color: "rgba(255,255,255,0.25)", fontSize: 11, margin: 0 }}>+{fmtPrice(Number(v.tip_amount))} tip</p> : <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 20, color: s.color, background: s.bg }}>{s.label}</span>}
                        </div>
                      </div>
                    );
                  })}
  ...
  return (
    <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div className="clients-kpi-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
        {[
          ...
          <div key={s.label} style={{ ...card, padding: "16px 18px" }}>
            ...
            {s.sub && <p style={{ color: "rgba(255,255,255,0.25)", fontSize: 11, margin: 0 }}>{s.sub}</p>}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "8px 12px", width: 240, flex: "1 1 200px", maxWidth: 320 }}>
          <Search size={13} color="rgba(255,255,255,0.3)" />
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search clients…" style={{ background: "none", border: "none", outline: "none", color: "rgb(250,250,250)", fontSize: 12.5, flex: 1, minWidth: 0 }} />
        </div>
                ...
                >
                  <td style={{ padding: "13px 18px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
                      <div style={{ position: "relative", flexShrink: 0 }}>
                        ...
                        <span style={{ position: "absolute", bottom: 0, right: 0, width: 9, height: 9, borderRadius: "50%", background: c.tag ? (CHURN_DOT[c.tag] ?? CHURN_DOT_DEFAULT) : CHURN_DOT_DEFAULT, border: "1.5px solid rgb(10,10,12)" }} />
                      </div>
                      <div style={{ minWidth: 0 }}>
                        <p style={{ color: "rgb(250,250,250)", fontSize: 13, fontWeight: 700, margin: "0 0 2px" }}>{c.name}</p>
                        <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 11.5, margin: 0 }}>{c.email ?? c.phone ?? ""}</p>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding: "13px 18px" }}>
                  ...
                  <td style={{ padding: "13px 18px" }}>
                    <div style={{ display: "flex", gap: 14 }}>
                      <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 12.5 }}><span style={{ color: "rgb(250,250,250)", fontWeight: 700 }}>{c.visits}</span> visits</span>
                      <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 12.5 }}><span style={{ color: "rgb(52,211,153)", fontWeight: 700 }}>{fmtPrice(c.ltv)}</span></span>
                      <span style={{ color: "rgba(255,255,255,0.35)", fontSize: 12.5 }}>{fmtDaysAgo(c.daysSinceLastVisit)}</span>
                    </div>
                  </td>
                  <td style={{ padding: "13px 18px", color: "rgb(250,250,250)", fontSize: 13, fontWeight: 600 }}>{fmtPrice(avgSpend)}</td>
                  <td style={{ padding: "13px 18px" }}>
                    {engaged
      ...
      <div className="clients-card-list" style={{ display: "none", flexDirection: "column", gap: 8 }}>
          ...
          return (
            <div key={c.key} onClick={() => setSelected(c)} style={{ ...card, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}>
              <div style={{ position: "relative", flexShrink: 0 }}>
                ...
                <span style={{ position: "absolute", bottom: 0, right: 0, width: 10, height: 10, borderRadius: "50%", background: c.tag ? (CHURN_DOT[c.tag] ?? CHURN_DOT_DEFAULT) : CHURN_DOT_DEFAULT, border: "1.5px solid rgb(10,10,12)" }} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                  <p style={{ color: "rgb(250,250,250)", fontSize: 13.5, fontWeight: 700, margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.name}</p>
                  {engaged && <Zap size={11} color="rgb(167,139,250)" style={{ flexShrink: 0 }} />}
                </div>
                <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 11.5, margin: 0 }}>{c.visits} visits · {fmtPrice(c.ltv)} · {fmtDaysAgo(c.daysSinceLastVisit)}</p>
              </div>
        ...
        {filtered.length === 0 && (
          <div style={{ ...card, padding: "40px 20px", textAlign: "center", color: "rgba(255,255,255,0.25)", fontSize: 13 }}>No clients found</div>
        )}
      </div>
