...
export default async function AdminPaymentsRiskPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        <h1 style={{ color: "rgb(250,250,250)", fontSize: 22, fontWeight: 800, margin: "0 0 4px", letterSpacing: "-0.03em" }}>Payments Risk</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
        {[
          ...
          <div key={s.label} style={{ ...card, padding: "20px 22px" }}>
            <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 13, fontWeight: 500, margin: "0 0 10px" }}>{s.label}</p>
            <p style={{ color: "rgb(245,245,252)", fontSize: 28, fontWeight: 800, letterSpacing: "-0.03em", margin: "0 0 6px" }}>{s.value}</p>
            <p style={{ color: "rgba(255,255,255,0.25)", fontSize: 12, margin: 0 }}>{s.sub}</p>
          </div>
        ))}
      </div>
      ...
      {disputes.length === 0 ? (
        <div style={{ ...card, minHeight: 170, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10 }}>
          <div style={{ width: 50, height: 50, borderRadius: "50%", background: "rgba(255,255,255,0.04)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <ShieldAlert size={22} color="rgba(255,255,255,0.2)" strokeWidth={1.4} />
          </div>
          <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 13, fontWeight: 600, margin: 0 }}>No disputes yet</p>
          <p style={{ color: "rgba(255,255,255,0.18)", fontSize: 12, margin: 0 }}>Real disputes will appear here once Stripe is connected</p>
        </div>
      ) : (
